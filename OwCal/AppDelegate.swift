...
//        dateCheckTimer = Timer.scheduledTimer(timeInterval: 3, target: self, selector: #selector(checkDateAndUpdate), userInfo: nil, repeats: true)
    ...
    private func fetchTodayDate() -> Date? {
        ...
        let todayDateTer = run_shell(launchPath: "/bin/bash", arguments: ["-c", "curl -s -m5 --head 'http://172.20.170.176/homepage/login.html' | grep -i 'date' | awk '{print $3$4$5}'"]).1.trimmingCharacters(in: .whitespacesAndNewlines)
